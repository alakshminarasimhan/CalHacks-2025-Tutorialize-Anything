...
export default function SavedStoryboardsPage() {
  ...
  const handleDelete = async (sessionId: string, url: string) => {
    ...
    try {
      const token = await getToken();
      const response = await fetch(`/api/storyboards/${sessionId}?url=${encodeURIComponent(url)}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
            ...
            {storyboards.map((storyboard) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => handleDelete(storyboard.sessionId, storyboard.url)}
