...
export default function TutorialViewer() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log('Original URL from session:', data.url);
        ...
        setSessionData(data); // Store the full session data including URL
        setLoading(false);
      } catch (err) {
  ...
  const handleRephrase = async () => {
    ...
    try {
      ...
      // ✅ Fetch full session data including original URL
      const sessionRes = await fetch(`/api/tutorial?sessionId=${sessionId}`);
      const sessionData = await sessionRes.json();

      console.log(">>> Saving with sessionData.url =", sessionData.url);
      ...
      const res = await fetch('/api/save-board', {
        ...
        body: JSON.stringify({
          ...
          sessionId,
          sessionData,          // ✅ includes correct url
          title: sessionData.title || `Tutorial - ${new URL(sessionData.url).hostname}`,
        }),
      });
