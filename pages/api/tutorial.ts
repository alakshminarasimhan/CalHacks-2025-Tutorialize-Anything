...
export default async function handler(
  ...
  const { url, style, voice, voiceId } = req.body;

  if (!url) {
    return res.status(400).json({ error: 'Missing URL' });
  }

  // Map voice selection to Fish.Audio voice IDs
  // Real voice IDs from Fish.Audio
  const voiceMapping: Record<string, string> = {
    'Default Male': '07424106e0ff4c58919e77d6f421c48b',
    'Default Female': '2a9605eeafe84974b5b20628d42c0060',
    'Minnie Mouse': 'ac04af95ebe747ceae442e5796e02729',
    'Kim Kardashian': 'f98232d43a2a41d8818ef9c0ee80b027',
    'Jake Paul': '874aa2258ca947fc9cd0a7902ae569ca',
    'Darth Vader': '3446a5f02a1c4d6fa6e85cf467302f4f',
    'Spongebob Squarepants': '54e3a85ac9594ffa83264b8a494b901b',
  };

  const selectedVoiceId = voice ? (voiceMapping[voice] || voiceId || '') : (voiceId || '');
  console.log(`Voice selected: "${voice}" -> Using voiceId: "${selectedVoiceId}"`);
  ...
  try {
    ...
    createSession(sessionId, {
      ...
      voiceId: selectedVoiceId,  // Store voice preference for later audio generation
    });

    console.log(`Session created with voice: ${voice} -> voiceId: ${selectedVoiceId}`);
