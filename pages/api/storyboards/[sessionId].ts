...
import { getStoryboard, deleteStoryboard } from '@/lib/dynamodb';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      // If authenticated, try DynamoDB first
      if (user) {
        storyboard = await getStoryboard(user.userId, sessionId);
      }
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      await deleteStoryboard(user.userId, sessionId);
